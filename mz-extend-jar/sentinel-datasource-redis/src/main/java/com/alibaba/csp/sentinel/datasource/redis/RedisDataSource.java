...
public class RedisDataSource<T> extends AbstractDataSource<String, T> {
    ...
    private StatefulRedisConnection<String, String> statefulRedisClients;
    private StatefulRedisClusterConnection<String, String> stringStatefulRedisClusterConnection;
    ...
    public String readSource() {
        ...
        if (redisClient != null) {
            if (statefulRedisClients == null || !statefulRedisClients.isOpen()){
                statefulRedisClients = redisClient.connect();
            }
            RedisCommands<String, String> stringRedisCommands = statefulRedisClients.sync();
            return stringRedisCommands.get(ruleKey);
        } else {
            if (stringStatefulRedisClusterConnection == null || !stringStatefulRedisClusterConnection.isOpen()){
                stringStatefulRedisClusterConnection = redisClusterClient.connect();
            }
            RedisAdvancedClusterCommands<String, String> stringRedisCommands = stringStatefulRedisClusterConnection.sync();
            return stringRedisCommands.get(ruleKey);
        }
    }

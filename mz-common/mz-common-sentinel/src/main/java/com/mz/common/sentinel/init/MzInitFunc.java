...
public class MzInitFunc implements ApplicationRunner {
    ...
    @Resource(name = "sentinel-json-flow-converter")
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        ReadableDataSource<String, List<FlowRule>> redisDataSource = new RedisDataSource<>(
                config,
                redisDataSourceProperties.getRuleKey(),
                redisDataSourceProperties.getChannel(),
                jsonFlowConverter
        );
        FlowRuleManager.register2Property(redisDataSource.getProperty());

        logger.info("{}：拉取sentinel规则成功",redisDataSourceProperties.getRuleKey());
    }
}

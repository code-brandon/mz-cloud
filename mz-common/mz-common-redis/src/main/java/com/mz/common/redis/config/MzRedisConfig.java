...
public class MzRedisConfig {
    ...
    @Bean("mzGzipRedisTemplate")
    public RedisTemplate<String, Object> gzipRedisTemplate(RedisConnectionFactory factory) {

        GzipJsonJacksonRedisSerializer jacksonSeial = new GzipJsonJacksonRedisSerializer(Object.class);
        ObjectMapper om = new ObjectMapper();
        // 指定要序列化的域，field,get和set,以及修饰符范围，ANY是都有包括private和public
        om.setVisibility(PropertyAccessor.ALL, JsonAutoDetect.Visibility.ANY);
        // 指定序列化输入的类型，类必须是非final修饰的，final修饰的类，比如String,Integer等会跑出异常
        ...
        template.setHashValueSerializer(jacksonSeial);
        template.afterPropertiesSet();
        log.info("MzGzipRedisTemplate配置类完成:{}",template.getClass().toString());
        return template;
    }
}
